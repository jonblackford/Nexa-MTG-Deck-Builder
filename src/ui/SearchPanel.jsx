...
export default function SearchPanel({ columns, commanderColumnId, onAddCard }) {
  ...
  const [selectedColId, setSelectedColId] = useState(columns?.[0]?.id ?? null)

  React.useEffect(() => {
    if (!selectedColId && columns && columns.length) setSelectedColId(columns[0].id)
  }, [columns])
  ...
  async function addToDefault(card) {
    setErr('')
    try {
      const colId = selectedColId || findColumnId(columns, categorizeByType(card))
          ...
          <div className="modal" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={runSearch} style={{ marginTop: 10 }}>
              ...
              <div style={{ marginTop: 8 }}>
                <label style={{ fontSize: 12 }} className="muted">Add to column</label>
